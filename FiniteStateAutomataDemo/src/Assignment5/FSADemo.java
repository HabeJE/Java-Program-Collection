...
public class FSADemo {	//Client class that contains main function. Creates a finite state automata, which accepts a grammar of ones and zeroes. Checks if a recieved string has an even numbers of both ones and zeroes, returning true if this is the case, and false if it is not.
	...
	public boolean withinLanguage(String string) {	//This boolean method is the actual check for if the received string is within the language of the automata.
		...
		for (int counter = 0; counter <= string.length() - 1; counter++)	
		{
			...
			if (c == '1') {	//if the character is one.
				this.oneInput();	//Relate it to the oneInput(), and execute that method for the JEAHFiniteStateAutomata object.
			}
			else {	//Otherwise, if the character is zero.
				this.zeroInput();	//Relate it to the zeroInput(), and execute that method for the JEAHFiniteStateAutomata object.
			}
		}
